"""Native cognis-connect emit for mevscope — forward findings to any platform.

Maps mevscope's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    mevscope ... --format json | mevscope-emit --to stix
    mevscope-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a mevscope record toward the Finding contract before normalize().
