...
**mevscope** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `mevscope` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
mevscope ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
mevscope ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
mevscope ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `mevscope` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="mevscope") for rec in records]   # records = your JSON output
